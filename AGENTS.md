...
This repo is a Claude Code plugin **and** an Obsidian vault that builds persistent, compounding knowledge bases using Andrej Karpathy's LLM Wiki pattern. It works with **any AI coding agent** that supports the Agent Skills standard, including Antigravity CLI (`agy`), Gemini CLI, Codex CLI, OpenCode, and similar. Antigravity CLI users: see [`ANTIGRAVITY.md`](ANTIGRAVITY.md) for the `agy`-specific entry point.
...
```bash
# Antigravity CLI (project-level skills dir + gemini-family global)
ln -s "$(pwd)/skills" .agents/skills
ln -s "$(pwd)/skills" ~/.gemini/skills/antigravity-obsidian
...
**Antigravity CLI:** after linking, run `agy inspect` to confirm the loaded
`AGENTS.md`, available Agent Skills, and MCP servers. See [`ANTIGRAVITY.md`](ANTIGRAVITY.md).
...
1. Read this file (`AGENTS.md`) and the project `CLAUDE.md` for full context (Antigravity CLI: also read `ANTIGRAVITY.md`)
